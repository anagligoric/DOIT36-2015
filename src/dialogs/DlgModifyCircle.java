...
public class DlgModifyCircle extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public DlgModifyCircle(Circle cr) {
		Point center = new Point(cr.getCenter().getX(), cr.getCenter().getY());
		circle = new Circle(center, cr.getR(), cr.getClrBorderColor(), cr.getClrInnerColor());
		chosenBorderColor = cr.getClrBorderColor();
		chosenInnerColor = cr.getClrInnerColor();
		...
		}
		{
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						if(Integer.parseInt(txtR.getText().toString())<=0) {
							...
							try {
								if (rdbtnMoveTo.isSelected()) {
									circle.moveTo(Integer.parseInt(txtX.getText()), Integer.parseInt(txtY.getText()));
								}
								if (rdbtnMoveBy.isSelected()) {
									circle.moveBy(Integer.parseInt(txtByX.getText()),
											Integer.parseInt(txtByY.getText()));
								}
								circle.setR(Integer.parseInt(txtR.getText().toString()));
								circle.setClrBorderColor(btnBorderColor.getSelectedColor());
								circle.setClrInnerColor(btnInnerColor.getSelectedColor());
			...
			txtX.setText(Integer.toString(cr.getCenter().getX()));
			txtY.setText(Integer.toString(cr.getCenter().getY()));
			txtR.setText(Integer.toString(cr.getR()));
		}
	}
