...
public class DlgModifyRectangle extends JDialog {
	...
	private JTextField txtX, txtHeight, txtWidth;
	 ...
	 * Create the dialog.
	 */
	public DlgModifyRectangle(Rectangle rc) {
		Point upperLeft = new Point(rc.getUpperLeft().getX(), rc.getUpperLeft().getY());
		System.out.println("NNNN" + upperLeft.toString());
		rectangle = new Rectangle(upperLeft, rc.getSideLength(), rc.getWidth(), rc.getClrBorderColor(), rc.getClrInnerColor());
		chosenBorderColor = rc.getClrBorderColor();
		chosenInnerColor = rc.getClrInnerColor();
		...
		{
			JLabel lblSideLength = new JLabel("Height:");
		...
		}
		{
			...
			gbc_lblChooseInnerColor.gridy = 9;
		...
		}
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						if(Integer.parseInt(txtHeight.getText().toString()) <= 0 || Integer.parseInt(txtWidth.getText().toString()) <= 0)	{
							JOptionPane.showMessageDialog(null, "Rectangle sides can not be negative", "Error", JOptionPane.ERROR_MESSAGE);;
						}else {
							try {
								if (rdbtnMoveTo.isSelected()) {
									rectangle.moveTo(Integer.parseInt(txtX.getText()), Integer.parseInt(txtY.getText()));
								}
								if (rdbtnMoveBy.isSelected()) {
									rectangle.moveBy(Integer.parseInt(txtByX.getText()),
											Integer.parseInt(txtByY.getText()));
								}
								rectangle.setSideLength(Integer.parseInt(txtHeight.getText().toString()));
								rectangle.setWidth(Integer.parseInt(txtWidth.getText().toString()));
								rectangle.setClrBorderColor(btnBorderColor.getSelectedColor());
								rectangle.setClrInnerColor(btnInnerColor.getSelectedColor());
			...
			txtX.setText(Integer.toString(rc.getUpperLeft().getX()));
			txtY.setText(Integer.toString(rc.getUpperLeft().getY()));
			txtHeight.setText(Integer.toString(rc.getSideLength()));
			txtWidth.setText(Integer.toString(rc.getWidth()));
		}
	}
